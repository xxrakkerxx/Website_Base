...
if ($sql->connect_error) {
  ...
  $update_data = "UPDATE participants SET STATUS='ENROLLED' WHERE ID =?";
  $process = $sql->prepare($update_data);
  $process->bind_param("i",$_POST['patient_id']);
  $process->execute();
  ...
  if ($process->execute() === TRUE) {
    ...
    if ($result_email->num_rows > 0 ) {
      ...
      $mail->msgHTML('Hello '.$row["FIRSTNAME"].' Your request for joining room number:['.$row["ROOM_CODE"].'] has been approved! you can now use your account and connect to your respective health admins.<br>
