...
<ul class="nav nav-tabs" id="tab-panel" role="tablist">
      ...
      <div class="tab-pane fade show active" id="personal-panel" role="tabpanel" aria-labelledby="details-tab">
        ...
        <input type="text"  required oninput="this.value = this.value.toUpperCase()" autocomplete="off" class="form-control " name="lname" id="lname" value="<?php echo  $_SESSION["admin_lname"]; ?>">
        <br>
        <label for="fname">First Name:</label>
        <input type="text"  oninput="this.value = this.value.toUpperCase()" class="form-control " name="fname" id="fname" value="<?php echo   $_SESSION["admin_fname"]; ?>"> 
        <br>
        <label for="mname">Middle Name:</label>
        <input type="text"  oninput="this.value = this.value.toUpperCase()" class="form-control " name="mname" id="mname" value="<?php echo  $_SESSION["admin_mname"]; ?>">
        <br>
        <label for="age">Age:</label>
        <input type="number"  max="60" readonly min="18" class="form-control " name="age" id="age" value="<?php echo   $_SESSION["admin_age"]; ?>">
        <br>
        ...
        <input type="date" max="2002-01-01" min="1960-01-01" onblur="agechecker()" class="form-control " name="bday" id="bday" value="<?php echo  $_SESSION["admin_bday"]; ?>">
        <hr>
        <br>
        <label for="add_community">Address:</label>
        <input type="text" oninput="this.value = this.value.toUpperCase()" class="form-control" name="add_community" id="add_community" value="<?php echo  $_SESSION["admin_comm"]; ?>">
        <br>
        <label for="add_community_name">Community Name:</label>
        <input type="text" oninput="this.value = this.value.toUpperCase()" class="form-control" name="add_community_name" id="add_community_name" value="<?php echo  $_SESSION["admin_comm_name"]; ?>">
        <br><br>
        ...
        <option id="approve">APPROVE</option>
        ...
        <input type="text" class="form-control " name="state" id="cty" value="<?php echo  $_SESSION["admin_cty"]; ?>">
        <br>
...
var text_zip = document.getElementById("zip");
     ...
     text_lname.addEventListener("keyup", function(event) {
      ...
      text_zip.addEventListener("keyup", function(event) {
        if (event.keyCode === 13) {
        event.preventDefault();
        document.getElementById("btn-save").click();
        }
...
$(document).ready(function(){
  ...
  $("#btn-save").click(function(){
    ...
    //var special_sess = "<?php $_SESSION["key"] = "access_granted"; echo $_SESSION["key"];?>"
   ...
   if ($.trim(lname)!='' && $.trim(fname)!='' && $.trim(mname)!='' && $.trim(room_code)!='' && $.trim(uname)!='' && $.trim(pword)!='' && $.trim(email)!='' && $.trim(age)!='' 
        && $.trim(add)!='' && $.trim(com_add)!='' && $.trim(cty)!='' && $.trim(zip)!='' && $.trim(state)!='' && $.trim(room_code)!='' && $.trim(status)!='') {
      ...
      $.ajax({
                  ...
                  com_add:com_add, state:state, cty:cty, zip:zip, room_code:room_code, uname:uname, pword:pword, email:email, status:status},
                  dataType:"text",
        ...
        //execute below line
        //imposibleng ma execute to haha LoL!
        //$("#load-spin").prop("hidden",false);  
...
if ($sql->connect_error) {
  ...
  if (isset($_POST['del-yes'])) {
    $del_query = "DELETE FROM admin WHERE ID='$_SESSION[admin_id]'"; //delete their record
    $del_msg = "DELETE FROM messages WHERE MESSAGE_ID='$_SESSION[admin_id]'"; //also their messages including the admin messages to them
